#' @title Decision Forest preferred-1 algorithm: Model training
...
#'
DFp1_train = function (X, Y, stop_step=5,
                     Max_tree=20, min_leaf=10, cp = 0.1,
  ...
  while(Curr_tree<Max_tree & stop_signal<stop_step & length(used_feat)>1 & stop_sign ==0 ){
    model = Con_DT(Train_data[,used_feat],Train_label,min_leaf,cp=cp)
  ...
  result = list(performance=consensus_result,pred=pred_accumulate,detail=Tree_model,models=models,Method=Method,cp=cp,Used_Feat = fin_feat_table)
  return(result)
}
